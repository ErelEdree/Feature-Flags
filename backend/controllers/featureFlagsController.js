...
const newFlag = new Flag({name, enabled, description});
await newFlag.save();
...
exports.updateFeatureFlag = async (req, res) => {
    try {
      const { name, enabled, description } = req.body;
      const flag = await Flag.findByIdAndUpdate(req.params.id, { name, enabled, description }, { new: true, runValidators: true });
      if (!flag) {
